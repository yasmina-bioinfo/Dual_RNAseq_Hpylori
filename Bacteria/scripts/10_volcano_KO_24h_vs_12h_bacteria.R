# Bacteria/scripts/10_volcano_KO_24h_vs_12h_bacteria.R
# Volcano plot — BACTERIA: KO 24h vs KO 12h

options(stringsAsFactors = FALSE)

res_csv <- file.path("results", "tables", "deseq_bact_KO_24h_vs_12h.csv")
stopifnot(file.exists(res_csv))

fig_dir <- file.path("results", "figures")
if (!dir.exists(fig_dir)) dir.create(fig_dir, recursive = TRUE)
out_png <- file.path(fig_dir, "volcano_bact_KO_24h_vs_12h.png")
...
plot(
  ...
  xlab = "log2 Fold Change (KO 24h vs 12h, bacteria)",
  ylab = "-log10 adjusted p-value",
  main = "Volcano — Bacteria KO: 24h vs 12h"
)
