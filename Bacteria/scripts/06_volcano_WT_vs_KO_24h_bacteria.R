# Bacteria/scripts/06_volcano_WT_vs_KO_24h_bacteria.R
# Volcano plot — BACTERIA: WT vs KO at 24h

options(stringsAsFactors = FALSE)

res_csv <- file.path("results", "tables", "deseq_bact_WT_vs_KO_24h.csv")
stopifnot(file.exists(res_csv))

fig_dir <- file.path("results", "figures")
if (!dir.exists(fig_dir)) dir.create(fig_dir, recursive = TRUE)
out_png <- file.path(fig_dir, "volcano_bact_WT_vs_KO_24h.png")
...
plot(
  ...
  xlab = "log2 Fold Change (WT vs KO, bacteria, 24h)",
  ylab = "-log10 adjusted p-value",
  main = "Volcano — Bacteria: WT vs KO at 24h"
)
