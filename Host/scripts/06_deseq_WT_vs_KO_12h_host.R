# scripts/06_deseq_WT_vs_KO_12h_host.R
# Differential expression (HOST): WT vs KO at 12h
...
res_csv <- file.path(out_dir, "deseq_WT_vs_KO_12h_host.csv")
...
# ---- Subset: infected at 12h, WT or KO ----
keep <- meta$condition == "infected" & meta$time == 12 & meta$strain %in% c("WT", "KO")
meta12 <- meta[keep, ]
counts12 <- counts[, meta12$sample_id]

# Safety checks
stopifnot(all(colnames(counts12) == meta12$sample_id))
stopifnot(all(meta12$time == 12))
...
message("✅ DESeq2 completed: HOST WT vs KO at 12h")
