# scripts/10_volcano_KO_24h_vs_12h_host.R
# Volcano plot — HOST: KO 24h vs KO 12h

options(stringsAsFactors = FALSE)

res_csv <- file.path("results", "tables", "deseq_KO_24h_vs_12h_host.csv")
stopifnot(file.exists(res_csv))

fig_dir <- file.path("results", "figures")
if (!dir.exists(fig_dir)) dir.create(fig_dir, recursive = TRUE)
out_png <- file.path(fig_dir, "volcano_KO_24h_vs_12h_host.png")
...
plot(
  ...
  xlab = "log2 Fold Change (KO 24h vs 12h)",
  ylab = "-log10 adjusted p-value",
  main = "Volcano — Host response: KO 24h vs KO 12h"
)
