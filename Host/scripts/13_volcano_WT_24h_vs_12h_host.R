# scripts/13_volcano_WT_24h_vs_12h_host.R
# Volcano plot — HOST: WT 24h vs WT 12h

options(stringsAsFactors = FALSE)

res_csv <- file.path("results", "tables", "deseq_WT_24h_vs_12h_host.csv")
stopifnot(file.exists(res_csv))

fig_dir <- file.path("results", "figures")
if (!dir.exists(fig_dir)) dir.create(fig_dir, recursive = TRUE)
out_png <- file.path(fig_dir, "volcano_WT_24h_vs_12h_host.png")
...
plot(
  ...
  xlab = "log2 Fold Change (WT 24h vs 12h)",
  ylab = "-log10 adjusted p-value",
  main = "Volcano — Host response: WT 24h vs WT 12h"
)
